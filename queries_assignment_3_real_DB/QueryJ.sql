...
*/
SELECT GENRE_ID, ART_ID, NAME
FROM(
  SELECT AG.ID_GENRE AS GENRE_ID, A.ID_ARTIST AS ART_ID, A.NAME AS NAME, COUNT(DISTINCT AT.ID_TRACK) AS COUNTT, ROW_NUMBER()OVER (PARTITION BY AG.ID_GENRE ORDER BY COUNT(DISTINCT AT.ID_TRACK) DESC) AS RN
...
WHERE RN=1
GROUP BY GENRE_ID, NAME, ART_ID, COUNTT;
