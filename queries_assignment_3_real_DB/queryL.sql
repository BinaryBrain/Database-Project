...
FROM (
SELECT ARE.NAME AS AREA_NAME, A.NAME AS ART_NAME, COUNT(AT.ID_TRACK) AS TRACKCOUNT, ROW_NUMBER()OVER (PARTITION BY ARE.ID_AREA, ARE.NAME ORDER BY COUNT(DISTINCT AT.ID_TRACK) DESC) AS RN
