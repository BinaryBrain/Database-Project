(function () {
	...
	var buttons = [
		...
		},
		{
			...
			table: "artist",
			request: "SELECT * FROM(SELECT A.ID_ARTIST, A.NAME, COUNT_TRACK/COUNT_RELEASE AS RATIO FROM(SELECT  AT.ID_ARTIST AS ART_ID, COUNT(DISTINCT AT.ID_TRACK) AS COUNT_TRACK FROM ARTIST_TRACK AT GROUP BY AT.ID_ARTIST) TR, (SELECT AT.ID_ARTIST AS ART_ID, COUNT(DISTINCT M.ID_RELEASE) AS COUNT_RELEASE FROM ARTIST_TRACK AT, TRACK T, MEDIUM M WHERE AT.ID_TRACK=T.ID_TRACK AND T.ID_MEDIUM=M.ID_MEDIUM GROUP BY AT.ID_ARTIST) ME, ARTIST A WHERE A.ID_ARTIST=TR.ART_ID AND A.ID_ARTIST =ME.ART_ID ORDER BY RATIO DESC)WHERE ROWNUM <=10;"
		},
		{
			name: "S) Hit artist and hit ability",
			table: "artist",
			request:"SELECT A.ID_ARTIST, A.NAME, AVG(COUNTM) FROM(SELECT AT.ID_ARTIST AS ART_ID,  R.NAME AS RECORD_NAME, COUNT(DISTINCT T.ID_MEDIUM) AS COUNTM FROM ARTIST_TRACK AT, RECORDING R, TRACK T WHERE T.ID_RECORDING=R.ID_RECORDING  AND AT.ID_TRACK=T.ID_TRACK GROUP BY AT.ID_ARTIST, R.NAME HAVING COUNT(DISTINCT T.ID_MEDIUM)>=100),ARTIST A WHERE ART_ID=A.ID_ARTIST GROUP BY A.ID_ARTIST, A.NAME HAVING COUNT(RECORD_NAME)>=10;"
			//timeout
		}
	]
