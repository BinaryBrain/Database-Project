(function () {
	...
	var buttons = [
		...
		},
		{
			...
			//I can't add the id_release
		},
		{
			name: "R) List 10 artists with the highest ratio",
			table: "artist",
			request: "SELECT * FROM(SELECT A.ID_ARTIST, A.NAME, COUNT_TRACK/COUNT_RELEASE AS RATIO FROM(SELECT  AT.ID_ARTIST AS ID_ARTIST, COUNT(DISTINCT AT.ID_TRACK) AS COUNT_TRACK FROM ARTIST_TRACK AT GROUP BY AT.ID_ARTIST) TR, (SELECT AT.ID_ARTIST AS ID_ARTIST, COUNT(DISTINCT M.ID_RELEASE) AS COUNT_RELEASE FROM ARTIST_TRACK AT, TRACK T, MEDIUM M WHERE AT.ID_TRACK=T.ID_TRACK AND T.ID_MEDIUM=M.ID_MEDIUM GROUP BY AT.ID_ARTIST) ME, ARTIST A WHERE A.ID_ARTIST=TR.ID_ARTIST AND A.ID_ARTIST =ME.ID_ARTIST ORDER BY RATIO DESC)WHERE ROWNUM <=10"
		},
		{
			name: "S) Hit artist and hit ability",
			table: "artist",
			request:"SELECT A.ID_ARTIST, A.NAME, AVG(COUNTM) FROM(SELECT AT.ID_ARTIST AS ID_ARTIST,  R.NAME AS RECORD_NAME, COUNT(DISTINCT T.ID_MEDIUM) AS COUNTM FROM ARTIST_TRACK AT, RECORDING R, TRACK T WHERE T.ID_RECORDING=R.ID_RECORDING  AND AT.ID_TRACK=T.ID_TRACK GROUP BY AT.ID_ARTIST, R.NAME HAVING COUNT(DISTINCT T.ID_MEDIUM)>=100),ARTIST A WHERE ID_ARTIST=A.ID_ARTIST GROUP BY A.ID_ARTIST, A.NAME HAVING COUNT(RECORD_NAME)>=10"
			//timeout
		}
	]
	...
	$(".output").on('click', '.result-links', function (event) {
		...
		if(table === 'artist') {
			...
			requests['artist'] = "SELECT DISTINCT a.NAME, a.ID_ARTIST, a.GENDER, a.ID_AREA, a.TYPE FROM artist a, track t, artist_track x WHERE x.ID_TRACK = t.ID_TRACK AND x.ID_ARTIST = a.ID_ARTIST AND t.ID_RECORDING = "+id
			...
			requests['artist'] = "SELECT DISTINCT a.NAME, a.ID_ARTIST, a.GENDER, a.ID_AREA, a.TYPE FROM artist a, medium m, track t, artist_track x WHERE t.ID_MEDIUM = m.ID_MEDIUM AND x.ID_TRACK = t.ID_TRACK AND x.ID_ARTIST = a.ID_ARTIST AND m.ID_RELEASE = "+id
